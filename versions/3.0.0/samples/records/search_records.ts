...
class SearchRecords {
    static async searchRecords(): Promise<void> {
        ...
        await paramInstance.add(ZOHOCRMSDK.Record.SearchRecordsParam.CRITERIA, "(Last_Name:starts_with:Last Name)");
        await paramInstance.add(ZOHOCRMSDK.Record.SearchRecordsParam.FIELDS, "Last_Name,First_Name,Company,Email,Phone,Lead_Status");
        await paramInstance.add(ZOHOCRMSDK.Record.SearchRecordsParam.PER_PAGE, 50);
        await paramInstance.add(ZOHOCRMSDK.Record.SearchRecordsParam.PAGE, 1);
        
        // Alternative search options (uncomment to use)
        // await paramInstance.add(ZOHOCRMSDK.Record.SearchRecordsParam.EMAIL, "example@email.com");
        // await paramInstance.add(ZOHOCRMSDK.Record.SearchRecordsParam.PHONE, "555-123-4567");
        // await paramInstance.add(ZOHOCRMSDK.Record.SearchRecordsParam.WORD, "technology");
        
        const response = await recordOperations.searchRecords(paramInstance, headerInstance);
        if (response != null) {
    ...
    public static async initializeAndCall() {
        ...
        await(new ZOHOCRMSDK.InitializeBuilder())
            ...
            .token(token)
            .initialize();
        await SearchRecords.searchRecords();
    }
}
