...
class DeleteRecords {
    static async deleteRecords(): Promise<void> {
        ...
        const recordIds = "1055806000028683001,1055806000028683002,1055806000028683003";
        await paramInstance.add(ZOHOCRMSDK.Record.DeleteRecordsParam.IDS, recordIds);
        
        // Add workflow trigger parameter (optional)
        await paramInstance.add(ZOHOCRMSDK.Record.DeleteRecordsParam.WF_TRIGGER, true);
        
        const response = await recordOperations.deleteRecords(paramInstance, headerInstance);
        if (response !== null) {
