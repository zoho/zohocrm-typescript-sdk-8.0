...
class DownloadBulkWriteResult {
    static async downloadBulkWriteResult(downloadUrl: string, destinationFolder: string): Promise<void> {
        const bulkWriteOperations = new ZOHOCRMSDK.BulkWrite.BulkWriteOperations();
        
        const response = await bulkWriteOperations.downloadBulkWriteResult(downloadUrl);
    ...
    public static async initializeAndCall() {
        ...
        await(new ZOHOCRMSDK.InitializeBuilder())
            ...
            .token(token)
            .initialize();
        let downloadUrl = "https://download-accl.zoho.in/v2/crm/60920/bulk-write/105582/10558002.zip"; // URL from job details response
