...
class UnShareEmails {
    ...
    static async unShareEmails(moduleAPIName : string, id : bigint) {
        ...
        const response: ZOHOCRMSDK.APIResponse<ZOHOCRMSDK.RecordShareEmail.ActionHandler.MasterModel> = await recordShareEmailOperations.unshareEmails(id);
            ...
            if (actionHandler instanceof ZOHOCRMSDK.RecordShareEmail.ActionWrapper) {
                ...
                for (let actionResponse of actionResponses) {
                    if (actionResponse instanceof ZOHOCRMSDK.RecordShareEmail.SuccessResponse) {
                        let successResponse:  ZOHOCRMSDK.RecordShareEmail.SuccessResponse = actionResponse;
                        ...
                        let exception:  ZOHOCRMSDK.RecordShareEmail.APIException = actionResponse;
                ...
                let exception:  ZOHOCRMSDK.RecordShareEmail.APIException = actionHandler;
                console.log("Status:", exception.getStatus().getValue());
                console.log("Code:", exception.getCode().getValue());
                console.log("Details:");
                let details = exception.getDetails();
    ...
    static async initializeAndCall() {
        ...
        let id = BigInt(323423421);
        await UnShareEmails.unShareEmails(moduleAPIName, id);
    }
}
UnShareEmails.initializeAndCall();