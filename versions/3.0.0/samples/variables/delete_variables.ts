...
class DeleteVariables {
    static async deleteVariables(): Promise<void> {
        const variablesOperations = new ZOHOCRMSDK.Variables.VariablesOperations();
        
        // Create parameter map for variable IDs to delete
        const parameterMap = new ZOHOCRMSDK.ParameterMap();
        await parameterMap.add(ZOHOCRMSDK.Variables.DeleteVariablesParam.IDS, "1055806000028714007,1055806000028714008");  // Replace with actual variable IDs
        
        const response = await variablesOperations.deleteVariables(parameterMap);
    ...
    public static async initializeAndCall() {
        ...
        await(new ZOHOCRMSDK.InitializeBuilder())
            ...
            .token(token)
            .initialize();
        await DeleteVariables.deleteVariables();
    }
}
