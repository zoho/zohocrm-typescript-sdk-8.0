...
class GetProfile {
    static async getProfile(): Promise<void> {
        ...
        const profileId = BigInt("1055806000000026011");
        const response = await profilesOperations.getProfile(profileId);
        if (response !== null) {
            ...
            if (responseHandler instanceof ZOHOCRMSDK.Profiles.ResponseWrapper) {
                ...
                if (profiles != null) {
                    for (let profile of profiles) {
                        ...
                        if (sections != null) {
                            console.log("Profile Sections: " + sections.length);
                            for (let section of sections) {
                                console.log("  Section Name: " + section.getName());
                                
                                const categories = section.getCategories();
                                if (categories != null) {
                                    console.log("  Section Categories Count: " + categories.length);
                                }
                            }
                        }
