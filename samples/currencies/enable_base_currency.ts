...
class EnableBaseCurrency {
    static async enableBaseCurrency(): Promise<void> {
        const currenciesOperations = new ZOHOCRMSDK.Currencies.CurrenciesOperations();
        
        const request = new ZOHOCRMSDK.Currencies.BaseCurrencyWrapper();
        
        const currency = new ZOHOCRMSDK.Currencies.BaseCurrency();
        ...
        const response = await currenciesOperations.enableBaseCurrency(request);
        ...
        if (response !== null) {
            ...
            if (actionHandler instanceof ZOHOCRMSDK.Currencies.BaseCurrencyActionWrapper) {
                const actionResponses = actionHandler.getBaseCurrency();
                if (actionResponses != null) {
                    if (actionResponses instanceof ZOHOCRMSDK.Currencies.SuccessResponse) {
                        const successResponse = actionResponses;
                    ...
                    else if (actionResponses instanceof ZOHOCRMSDK.Currencies.APIException) {
                        const exception = actionResponses;
    ...
    public static async initializeAndCall() {
        ...
        await(new ZOHOCRMSDK.InitializeBuilder())
            ...
            .token(token)
            .initialize();
        await EnableBaseCurrency.enableBaseCurrency();
    }
}

EnableBaseCurrency.initializeAndCall();