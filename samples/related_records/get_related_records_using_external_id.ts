...
class GetRelatedRecordsUsingExternalId {
    static async getRelatedRecordsUsingExternalId(): Promise<void> {
        const moduleAPIName = "Accounts";
        const relatedListAPIName = "Contacts";
        const externalValue = "EXTERNAL_ACCOUNT_001";
        ...
        const fieldNames = ["Last_Name", "Email", "Phone", "Account_Name"];
        await paramInstance.add(ZOHOCRMSDK.RelatedRecords.GetRelatedRecordsUsingExternalIDParam.FIELDS, fieldNames.toString());
        
        // Add specific record IDs if needed
        const relatedRecordIds = ["4409002306004", "4409002306005"];
        await paramInstance.add(ZOHOCRMSDK.RelatedRecords.GetRelatedRecordsUsingExternalIDParam.IDS, relatedRecordIds.toString());
        
        // Add headers
        await headerInstance.add(ZOHOCRMSDK.RelatedRecords.GetRelatedRecordsUsingExternalIDHeader.IF_MODIFIED_SINCE, new Date("2020-06-01T00:00:00+00:00"));
        await headerInstance.add(ZOHOCRMSDK.RelatedRecords.GetRelatedRecordsUsingExternalIDHeader.X_EXTERNAL, "Accounts.External_Account_ID");
        
        const response = await relatedRecordsOperations.getRelatedRecordsUsingExternalId(externalValue, paramInstance, headerInstance);
    ...
    public static async initializeAndCall() {
        ...
        await(new ZOHOCRMSDK.InitializeBuilder())
            ...
            .token(token)
            .initialize();
        await GetRelatedRecordsUsingExternalId.getRelatedRecordsUsingExternalId();
    }
}
