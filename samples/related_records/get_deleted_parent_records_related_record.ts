...
class GetDeletedParentRecordsRelatedRecord {
    static async getDeletedParentRecordsRelatedRecord(): Promise<void> {
        ...
        await paramInstance.add(ZOHOCRMSDK.RelatedRecords.GetDeletedParentRecordsRelatedRecordParam.PER_PAGE, 200);
        
        const fieldNames = ["Last_Name", "Email", "Phone", "Account_Name"];
        // await paramInstance.add(ZOHOCRMSDK.RelatedRecords.GetDeletedParentRecordsRelatedRecordParam.FIELDS, fieldNames.toString());
        
        // Add specific record IDs if needed
        const relatedRecordIds = ["1055806000007081077", "4409002306005"];
        await paramInstance.add(ZOHOCRMSDK.RelatedRecords.GetDeletedParentRecordsRelatedRecordParam.IDS, relatedRecordIds.toString());
        
        const response = await relatedRecordsOperations.getDeletedParentRecordsRelatedRecord(recordId, paramInstance);
    ...
    public static async initializeAndCall() {
        ...
        await(new ZOHOCRMSDK.InitializeBuilder())
            ...
            .token(token)
            .initialize();
        await GetDeletedParentRecordsRelatedRecord.getDeletedParentRecordsRelatedRecord();
    }
}

GetDeletedParentRecordsRelatedRecord.initializeAndCall();