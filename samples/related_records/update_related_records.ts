...
class UpdateRelatedRecords {
    static async updateRelatedRecords(): Promise<void> {
        ...
        record1.setId(BigInt("1055806000005356009"));
        record1.addKeyValue("list_price", 50.56);
        recordsArray.push(record1);
        request.setData(recordsArray);
        
        const response = await relatedRecordsOperations.updateRelatedRecords(recordId, request, headerInstance);
    ...
    public static async initializeAndCall() {
        ...
        await(new ZOHOCRMSDK.InitializeBuilder())
            ...
            .token(token)
            .initialize();
        await UpdateRelatedRecords.updateRelatedRecords();
    }
}
