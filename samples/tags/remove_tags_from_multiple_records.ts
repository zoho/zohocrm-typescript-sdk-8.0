...
class RemoveTagsFromMultipleRecords {
    static async removeTagsFromMultipleRecords(): Promise<void> {
        ...
        const tag2 = new ZOHOCRMSDK.Tags.ExistingTag();
        tag2.setName("Bulk Campaign1");
        tagArray.push(tag2);

        const tag3 = new ZOHOCRMSDK.Tags.ExistingTag();
        ...
        const response = await tagsOperations.removeTagsFromMultipleRecords(moduleAPIName, requestWrapper, paramInstance);
    ...
    public static async initializeAndCall() {
        ...
        await(new ZOHOCRMSDK.InitializeBuilder())
            ...
            .token(token)
            .initialize();
        await RemoveTagsFromMultipleRecords.removeTagsFromMultipleRecords();
    }
}
