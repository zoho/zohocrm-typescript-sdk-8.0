...
class UpdateSharePermissions {
    static async updateSharePermissions(): Promise<void> {
        ...
        const response = await shareRecordsOperations.updateSharePermissions(bodyWrapper);
            ...
            if (actionHandler instanceof ZOHOCRMSDK.ShareRecords.ActionWrapper) {
                ...
                if (actionResponses !== null) {
                    for (let i = 0; i < actionResponses.length; i++) {
                        ...
                        console.log(`\n--- Update Permission Response ${i + 1} ---`);
                        ...
                        if (actionResponse instanceof ZOHOCRMSDK.ShareRecords.SuccessResponse) {
                            ...
                            // Check if permissions were successfully updated
                            if (successResponse.getStatus().getValue() === "success") {
                                console.log("✓ Share permissions successfully updated");
                            }
                        }
    ...
    public static async initializeAndCall() {
        ...
        await(new ZOHOCRMSDK.InitializeBuilder())
            ...
            .token(token)
            .initialize();
        await UpdateSharePermissions.updateSharePermissions();
    }
}
